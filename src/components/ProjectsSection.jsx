...
import { Layers, MapPin, Compass, AlertTriangle, ShieldAlert, Droplet, ArrowRight } from "lucide-react";
...
export default function ProjectsSection() {
  const projects = [
    ...
    },
    {
      ...
      subtitle: "GIS-Based Multi-Criteria Site Analysis",
      role: "Remote Sensing & GIS Analyst",
      desc: "Identified optimal DEM creation sites within the Kanshabati River Basin using nine spatial criteria and weighted overlay MCDM analysis, classifying the basin into five suitability zones.",
      icon: <Compass className="w-6 h-6 text-brand-red" />,
      tags: ["MCDM / AHP", "Weighted Overlay", "Raster Analysis", "DEM Processing"],
    ...
    <section id="projects" className="relative w-full min-h-screen bg-black py-24 md:py-32 flex flex-col justify-center">
      ...
      <div className="max-w-7xl mx-auto w-full px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, idx) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="group relative bg-[#080808] border border-white/10 hover:border-brand-red/50 rounded-2xl flex flex-col justify-between transition-all duration-300 shadow-2xl overflow-hidden"
            >
              {/* Hover glow */}
              <div className="absolute inset-0 bg-gradient-to-b from-brand-red/0 to-brand-red/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />

              {/* Card body — clickable area */}
              <Link
                to={`/projects/${project.id}`}
                className="block p-6 md:p-8 flex-1"
              >
                <div>
                  {/* Top Info */}
              ...
              <div className="px-6 md:px-8 pb-6 md:pb-8 pt-0">
                <Link
                  to={`/projects/${project.id}`}
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-white/5 border border-white/10 hover:bg-brand-red hover:border-brand-red text-white/60 hover:text-white text-xs font-bold uppercase tracking-widest transition-all duration-300 group/btn"
                >
                  View Details
        ...
        >
          <Link
            to="/projects"
            className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-white text-black font-bold text-sm uppercase tracking-widest hover:bg-brand-red hover:text-white transition-all duration-300 shadow-[0_4px_20px_rgba(255,255,255,0.15)] hover:shadow-[0_4px_20px_rgba(255,42,42,0.3)] hover:scale-105"
          >
            View All Projects
